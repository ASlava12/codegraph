...
The first implementation stores whole-graph JSON cache records keyed by root path and scan options, including the file-size budget. A fingerprint records scanned relative paths, file sizes, and modification times; a cache hit is only used when the current fingerprint matches the stored record. Cache diff reports compare the stored and current fingerprints so CLI, API, and web users can see added, removed, and modified files behind a miss. This is deliberately conservative: it accelerates repeated API and UI scans without yet promising partial graph reuse.
...
- expose scan budget configuration consistently across API, web, and source search workflows
