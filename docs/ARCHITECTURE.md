...
`codegraph-indexer` walks project roots, applies ignore rules and scan budgets, invokes parsers, and assembles graph fragments. The crate is organized by feature area: `options` (scan policy), `context` (shared indexing state and pending queues), `scan` (walking and per-file dispatch), `walk` (ignore rules), `parse_cache`, `detectors` (script/framework/platform-channel facts), `manifests` (package ecosystems), `runtime` (Make/Docker/CI/Kubernetes surfaces), `frameworks` (route/config conventions), `imports` (local import resolution), `docs_ingest` (Markdown), `sql`, `knowledge` (rationale/ORM/MCP facts), `rules` (user rules and annotations), and `resolve` (post-scan resolution passes).

It currently builds file, symbol, import, local import resolution, call, config, environment, error-flow, manifest entrypoint, script entrypoint, framework route entrypoint, custom rule violation, user annotation, and package dependency graph facts from syntax-level parser output, project manifests, framework conventions, `.codegraph/config.toml`, `.codegraph/rules.toml`, and `.codegraph/annotations.toml`. Repository scan policy supports ignored names and ignored project-relative globs so generated files, fixtures, and build artifacts can be excluded before parsing, source search, and cache fingerprinting. Explicit local import/include paths are resolved after the full scan so relative JavaScript/TypeScript imports and CommonJS requires, Python relative imports, quoted C/C++ includes, PHP include/require paths, Bash source paths, and common Rust module paths can point at files regardless of filesystem walk order. PHP namespace imports are also indexed as external import facts for Composer dependency analysis. Python absolute imports, Go module-local imports, and C/C++ includes through CMake or `compile_commands.json` include directories are also marked local only when they resolve to scanned project files, preserving dependency warnings for unresolved external packages. Manifest dependencies are normalized by ecosystem-specific package identity, including Cargo, npm `package.json`/`package-lock.json`/`pnpm-lock.yaml`, Go modules, Python packaging through `requirements.txt`, `pyproject.toml`, `setup.py`, `setup.cfg`, and `Pipfile`, Composer `composer.json`/`composer.lock`, vcpkg, Conan manifests, and CMake `find_package(...)` declarations, so multiple manifest files can point at the same external package node while preserving declaration kind and raw version or constraint details on each edge. Go `// indirect` requirements are preserved as `indirect` dependency edges so imports that should have been promoted to direct requirements are explainable. Poetry dependency groups are scoped as dev, test, or optional manifest edges so non-runtime Python tooling remains visible without looking like production runtime requirements. Cargo workspace dependency declarations resolve `workspace = true` through root `[workspace.dependencies]` when a version constraint is declared, and path-only workspace dependencies omit version metadata. npm lockfile dependencies read direct root package declarations from package-lock v2/v3 `packages[""]` and direct pnpm importer declarations, attaching resolved package versions while avoiding transitive dependency noise. Composer lockfile dependencies read installed runtime and dev packages with locked package versions. Manifest entrypoints include Cargo, npm, Go, Python `pyproject.toml`, `setup.py`, and `setup.cfg` console scripts, Composer, and CMake `add_executable` declarations. Shebang entrypoints cover Bash, Python, Node.js, and PHP scripts, including extensionless CLI files whose parser language is inferred from the shebang. Files above the configured scan file-size budget are not read, but source and manifest files remain visible as `file` nodes with `skipped_reason=max_file_size` metadata so missing symbol facts are explainable. Framework route entrypoints cover common FastAPI/Flask-style Python decorators, Express-style JavaScript/TypeScript routes, Axum-style Rust routes, Go `HandleFunc`/router calls, and PHP route attributes. User annotations attach `annotation.*` metadata to matching nodes so repository-specific ownership, domain, criticality, and review context can travel through CLI, API, web, and agent graph queries. Manifest entrypoints are resolved after the full scan so path and function targets can be linked regardless of filesystem walk order. Future implementations should merge stronger semantic facts from LSP and compiler layers.
...
- language dependency matrices that group dependency edges by source and target language
...
- plan semantic enrichment work by language, LSP capability, installed-server status, and capped concrete work items with stable ids, priorities, reasons, agent-facing filters, and server-grouped execution batches
- launch ready language-server batches over stdio from the CLI semantic runner
...
`codegraph-server` exposes the graph over HTTP and serves the embedded browser UI from `codegraph-web`. The binary is organized by feature area: `limits` (runtime constants and embedded assets), `contracts` (request/response structs and app state), `middleware` (auth, headers, logging, static assets), `jobs` (async scan/semantic job lifecycle), `system_handlers`, `semantic_handlers`, `analysis_handlers`, `params` (request parsing and the API error contract), and `schema` (the machine-readable `/api/schema` contract); `main.rs` keeps router wiring and shutdown. The web UI sources live as ordered modules in `codegraph-web/static/js/` (i18n data, state, DOM wiring, locale, deep links, runtime panels, overview, investigation panels, refactoring panels, results, filters, canvas, node cards, interactions, Flow view); the server build script concatenates them into the single `app.js` the browser loads, so lexicographic file order preserves the original statement order.
...
Current call, config, environment, and error-flow facts are `heuristic`: they are resolved by syntax patterns and simple-name matching. Explicit local import/include file edges are `syntactic` because they come from source-level paths. Package manifest dependencies and manifest-defined entrypoints are `exact` because they come from declared project metadata; package nodes expose `package_id`, and `depends_on` edge metadata records the declaration kind and raw version constraint when available, including resolved Cargo workspace constraints, npm and pnpm lockfile package versions, setup.py/setup.cfg requirement strings, Pipfile constraints, Composer lockfile package versions, vcpkg override versions, Conan reference versions, and CMake `find_package` versions. Dependency edges with version metadata also expose `dependency_version_kind=constraint` or `locked`; conflict insights compare constraints and ignore lockfile-only resolved versions. Entrypoint target `references` edges carry their own confidence: direct manifest paths such as Cargo target files, Go `main.go` files, Composer binaries, CMake executable source files, and shebang script files are exact, syntax-level function matches are syntactic, and command parsing or Python module-to-path mapping is heuristic.

Entrypoint trace reports run the normal outgoing traversal for all matching startup nodes so humans and agents can compare startup flows without manually copying labels. Dependent traces run the same dependency edge rules in reverse, giving humans and agents an impact-analysis subgraph for callers, importers, config readers, and other incoming dependents. Neighborhood queries expand local incoming, outgoing, or bidirectional context around a node with edge kind and confidence filters, giving agents a compact subgraph without reading a full repository graph. Config traces use those existing graph facts to match `config` and `environment` targets, list direct readers, and find shortest known upstream entrypoint paths before the final read edge. Framework config convention facts add syntactic `config` nodes and `reads_config` edges for common settings files and framework-specific config calls, with metadata recording the framework and config kind. Error traces do the same for `may_error` edges, connecting potential error/exception constructs back to their direct source and upstream entrypoint path when one is known.

Edge queries can filter by confidence, relation, source, and other metadata; the web UI displays confidence and provenance metadata on edge rows and exposes edge relation/source facets from the overview. Entrypoint resolution insights warn when a manifest-declared startup target cannot be linked to any scanned file or function. Dependency consistency insights compare syntactic external imports with declared package nodes, report likely undeclared package usage, flag runtime manifest dependencies with no matching import, warn when the same manifest package is declared with conflicting constraints, and catch production imports of packages declared only in non-runtime scopes such as Go `// indirect` requirements. Composer consistency checks map common PHP namespace imports such as `Monolog\*`, `Symfony\Component\Console\*`, `PHPUnit\Framework\*`, `GuzzleHttp\*`, and `Psr\Log\*` to package identities when comparing code usage with `composer.json` and `composer.lock`. Local import insights warn when an explicit local import/include path cannot be linked to a scanned file. Cross-language dependency insights flag heuristic or unknown-confidence edges between different source languages so mixed repositories can prioritize semantic review at integration boundaries. Framework route insights warn when the same HTTP method/path is declared by multiple detected route entrypoints and when a route names a handler that cannot be linked to a scanned function. Config reachability insights warn when a config or environment read exists in a reader that is not reachable from any detected entrypoint. Error reachability insights warn when a potential error/exception flow starts in code that is not reachable from any detected entrypoint. Source reachability insights flag non-test source files that contain code symbols but are not reached directly or through a contained symbol from any detected entrypoint. The web UI loads insight reports from server analysis, so project-wide findings remain visible while the canvas displays a paged graph slice. Future LSP, framework, and compiler integrations should upgrade resolvable code edges to `semantic` or `exact`.
