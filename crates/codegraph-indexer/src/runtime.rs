...
pub(crate) fn manifest_dependencies(
    ...
    match path.file_name().and_then(|name| name.to_str()) {
        ...
        Some("Project.toml") => julia_project_dependencies(source),
        Some("DESCRIPTION") => r_description_dependencies(source),
        Some("build.sbt") => sbt_dependencies(source),
        Some("build.zig.zon") => zig_zon_dependencies(source),
