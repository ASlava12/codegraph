...
- A go package tells two declarations of a name apart. The duplicate-label rule was already refined once -- a name each owner declares once is not a duplicate -- and the same refinement was owed to the package: `Add` in five of terraform's packages is five functions, and gqlgen writes a `queryResolver` per example. 3433 of the corpus's 3477 go groups have every declaration in its own directory, 98%, and no other language is above 53%, so this is go's rule rather than a general one about directories -- ocaml sits at 7% and python at 12%, where a repeated name really is in one place. terraform's duplicates fall from 718 to 52 and gqlgen's from 727 to 342, 1052 findings in all, with warnings unchanged at 329 and per project and no project gaining one. The view groups by the same three things, which the parity check on gqlgen now covers, and a fixture on each side pins two packages declaring it once against one package declaring it twice.
- A QuickCheck property is run by its module. shellcheck held the corpus's largest pile of `orphan_function` at 2756, and 2252 of its 2955 uncalled haskell definitions are `prop_*` -- properties `$(forAllProperties)` collects through Template Haskell, so the harness runs each one and no edge records it, exactly as `#[test]` does. Every one of the 2252 sits in one of the 11 files that collect them, which is why the rule asks the file rather than trusting the prefix: a `prop_` name in a module that collects nothing is a name. shellcheck falls from 3650 findings to 1397, 2253 in all, with warnings unchanged at 7 and at 329 across the corpus, and it is the only project that moves because it is the only haskell one. The first version marked 491 of the 2252: `forAllProperties` contains `AllProperties`, not `allProperties`, so only the modules importing `quickCheckAll` matched.
- A truncated query says what widens it. `query` is the one command that takes no `--limit`, because the bound belongs to the expression -- `nodes kind:function limit:400` -- and clap rejects the flag a reader reaches for first; the help's fourth example does show `limit:20`, so it is documented, but the answer itself said `truncated: true` and the totals and stopped there. `notes` is where a query says what it decided for itself, and how many to show is such a decision: a truncated answer now reads `showing 100 of 3919 nodes; \`limit:\` in the expression decides how many, and it is 100 here`, naming edges only when the query matched any and saying nothing when nothing was cut. Added once at `query_graph`, so every one of its twenty-three commands says it. The corpus sweep is byte-identical: a note is not a finding.
- A shell command is the environment's when it is not the script's. A shell has three places a command can come from -- a function the script declares, the shell itself, and PATH -- and there is no fourth, so `unresolved` there says a resolver failed where nothing was ever present to find. Not one of redis's 424 or shellcheck's 58 unresolved shell calls named a function either project declares, which is what makes the rule complete rather than a guess. The shell's own vocabulary was 20 names and missed `pwd`, `:`, `break`, `continue` and `command` among others -- 54 of redis's calls by themselves -- and the rest is PATH: `cat`, `sed`, `git`, `awk`. redis, shellcheck, kong and dune now have zero unresolved shell calls where they had 424, 58, 199 and 446; 458 findings go across 17 projects, no project gains one and warnings are unchanged at 329 and per project.
- A nix file takes `lib` and `pkgs` from whoever imports it. A nix file is a function, and `{ config, lib, pkgs, ... }:` at the top says what its caller supplies, so `lib.optionalAttrs` names something inside a value the file was handed -- nothing in the file, and nothing in the project, can be looked up for it. home-manager writes 1834 such calls, 1505 through `lib` and 311 through `pkgs`, and not one through `config`, where the claim would have been weaker. They read as `local_value` now, which is what a call through a bound value already means everywhere else; the resolution stays `unresolved` on purpose, because `external` would claim to know which package answers and this only knows that this project does not. home-manager falls from 1423 findings to 1293 and dune loses 12, with warnings unchanged at 7 and 31 and at 329 across the corpus. A parameter's default can itself be a set -- `pkgs ? import <nixpkgs> { }` -- so the brace that ends the list is the one that returns to depth zero.
- testthat hands an R suite its assertions. R was the last language above 40% unresolved after the shell and nix rules, and dplyr's 3069 unresolved calls split cleanly: 418 are testthat -- every assertion an `expect_*`, every block named outright, and all 418 under `tests/` -- and 189 are `pkg::fn`, where the source itself names the package that answers. Both now say so: unresolved falls to 2454, builtin rises by 426 and external by 189, and nothing that was resolved or ambiguous moves. The rest is `import(rlang)` -- a whole package taken in without naming a single function -- so which package answers `abort` or `caller_env` is not something this graph can say, and it does not claim to. dplyr falls from 929 findings to 888 and is the only project that moves; warnings are unchanged at 6 and at 329 across the corpus.
- An erlang call names the module that answers it. `gun:open(..)` says which module, and a module the project has no file for is a dependency's or OTP's -- and OTP is already answered before this point, with not one of cowboy's 3936 unresolved erlang calls naming an OTP module. What is left is 1764 calls to `gun`, `ranch`, `cow_hpack` and `quicer`, every one reported as a resolver failure. A capitalised head is an erlang variable holding a module name and says nothing about where it points, so 35 of those are left alone; 120 that name a module the project does have are a resolution gap and stay unresolved rather than being papered over. cowboy's unresolved calls fall 3936 to 2173 and its external ones rise 0 to 1763, with resolved, ambiguous and builtin untouched; 113 findings go, warnings unchanged at 0 and at 329 across the corpus. The refreshed language table has bash at 0% unresolved, rust down from 43% to 37.8% and R from 45.5% to 36.4%.
- The banner is in the opening comment, however long that is. The generated-file rule read the first six lines, and ffigen writes seven `// ignore_for_file:` lines before `// AUTO GENERATED FILE, DO NOT EDIT.` -- missed by two, which left 2457 calls in one dart file reading as a person's. Reading the whole leading comment block instead stops at the first line of code, so it also stops mistaking a banner a file emits as data for a statement about the file: kong returns a `DO NOT EDIT` header as a lua long string and is no longer marked. Two measurements shaped it: the block window lost dune's `(* Do not edit.` until OCaml comments were accepted, and eshop's and Newtonsoft.Json's nine `.Designer.cs` until the byte-order mark was stripped. Across the corpus 371 files say it now against 365, and 941 findings go -- http 2206 to 1434, redis 169 -- with warnings unchanged at 329 and per project and no project gaining one.
- A generator that names itself at the head of the line has said so. Not every generator shouts: oscar's 95 migrations under `src/oscar/apps/*/migrations/` say only `# Generated by Django 1.10.6 on 2017-03-30 14:35`, and mastodon's `db/schema.rb` says `This file is auto-generated from the current state of the database`. The discriminator is position rather than wording -- the same words inside a sentence are prose, and dplyr's `R/colwise.R` documents "a list of columns generated by [vars()]" in a doc comment, which a substring test would have marked. A comment line that begins `generated by`, `generated code` or `auto-generated` is a banner; oscar goes from 0 marked files to 130 and dplyr's prose stays unmarked. 59 findings go, warnings are unchanged at 329 and per project, and no project gains one.
- The two directions of a dependency question are told apart. `what depends on X` and `what does X depend on` are opposite questions, and the reverse rule declined the second on purpose -- with nothing picking it up, so it fell through to the package rule and answered `packages package:scan_project`, which is 0 nodes, where what that function reaches is 101. Both rules read one phrase now, so neither can drift into the other's half. Two things the first version got wrong and measurement caught: `зависит от` alone is the *reverse* direction in russian -- `кто зависит от X` -- and only the question word makes it forward, and `what does this project depend on` names no symbol, so reading its first candidate as one answered the package question with a search for a function called `project`. The forward rule takes a named subject only.
- A question verb is not a name to search for, and a cycle is not a package. Two more `ask` routings, found by running sixteen question shapes and reading what came back. "Show me the dependency cycles" was answered with this project's 258 packages, because a cycle is a kind of dependency and the package rule is checked first; naming the cycle settles it now, and both rules read one phrase so neither can drift. "Which types does insights use" ended on its verb, so the anchor guesser took `use` -- and the caller recognised it as a question verb and then handed that very word to the widened plan, which searched the project for it and answered 1017 nodes. The caller holds the graph, so it now prefers a word the project really names: `insights`, 770 nodes. A name the project does not have is still searched, which is what the widened plan is for.
- A node card takes the label its neighbours take. `impact main` and `journey --from main` both resolve a label and say which of the four definitions they picked; the card answered `invalid node id `main`` -- to a name the project has, and to the very labels its own suggested commands hand back. The MCP tool had taken labels all along, so the CLI and `/api/node-card` were the two that had not. Both resolve one now, through the ranking every query already uses, and the card carries a `notes` field that says which definition was chosen when several answer to the name. A well-formed id that names nothing is still a 404 rather than a bad request: the distinction between "this graph has no such node" and "that is not a name anyone wrote" is what the server test caught when the first version collapsed them.
- Every answer about a shared label says which one. Auditing the property rather than the command: all twelve combinations of the six node-taking commands with a label and with a durable id now resolve, and six of the seven print the sentence `impact` prints when several definitions answer to the name. The refactor bundle was the seventh -- it carried that sentence inside its `impact` and `dependencies` sections and nowhere a reader of the bundle looks, though the bundle is the thing advertised as one call for the whole context. It surfaces the same sentence at the top now, stays silent when only one definition answers, and a test pins both halves so a new surface has to opt in rather than quietly omit it.
- `use std::env;` binds a module, and a call is written through it. The rust import rule took only capitalised names from a `use`, reasoning that a lowercase one binds a function whose calls carry no qualifier -- which is true of `min` and false of every module. `doctor`, this project's own graph self-check, is what found it: it reported `env::temp_dir` in `codegraph-lsp` as a definition calling itself, because the bare method name matched the project's own `fn temp_dir` and that function is the one containing the call. 1194 calls on this repository go through a lowercase imported module, `fs` alone 1172; unresolved falls 22720 to 21534 and builtin rises by the same, one false edge goes, and `doctor`'s self-call count drops from 7 to 6. A lowercase name that binds a function instead is inert here, since `min(..)` carries no qualifier to look up. ripgrep loses 16 findings and serde 9, warnings unchanged at 329 and per project.
- An ocaml module call is not answered by a same-named local. `doctor`, run across the corpus for the first time, reports 0 dangling and 0 duplicate edges everywhere and 4311 definitions that appear to call themselves through a receiver -- 1027 on dune, 822 on cats. On dune 4778 module-qualified calls were answered by a same-file function of the bare name; 2412 of those name a module the file declares inside itself, where that answer is right, and 2366 name a module the file neither is nor declares, where `List.map` was being resolved to whatever local `map` the caller happened to sit beside. Those fall through now: dune's self-calls drop from 1027 to 241, 340 wrong edges go and 981 calls become an honest ambiguity, and `List.map` keeps resolving inside opam's `opamStd.ml` and dune's `boot/duneboot.ml`, which both write `module List = struct .. end` before calling it. The corpus baseline moves 329 to 330, the first rise this run: dune surfaces a cycle warning that rests on a false edge this change did not create -- `inotify.ml:75` calls the vendored ocaml-inotify's `Inotify.add_watch`, which matched dune's own `add_watch` because the vendored file does not declare that name either. That class is one call on dune; the twenty other cross-file resolutions of a self-named module are correct, reaching `vendor/ocaml-inotify/src/inotify.ml` and `otherlibs/stdune/src/path.ml`.
- A module answers for its own files and for what they include, and for nothing else. The commit before this one moved the corpus baseline from 329 to 330 by surfacing a cycle warning that rested on a false edge, and chasing that edge to the end is what produced this rule. dune answers 20163 of its 21194 module-qualified calls inside the module's closure -- the file named after it, plus whatever that file `include`s, which is how `Fiber.return` is correctly `core.ml`'s. The 796 that landed outside were `String.sub` in bigstringaf and `Unix.lstat` in stdune's `path.ml`. A module with no file here at all is the standard library's: 890 more calls, 237 of them answered outright by an unrelated file -- `Printf.printf` by stdune's `console.ml` 161 times. And a module declared inside a file is that file's own -- from anywhere else it is `Csexp_rpc.Unix`, not `Unix` -- which is what let `csexp_rpc.ml` answer `Unix.close` for `fd.ml` and close the cycle that started this. 1257 wrong resolutions go, `String.concat` alone 194 of them, every one pointing at the vendored `pp.ml` while stdune's `string.ml` is `include Stdlib.StringLabels`. dune's self-calls fall from 1027 to 212, its ambiguities from 3688 to 1150, the corpus baseline is back to 329 and dune's warnings are the same ones it started with. Its finding count rises by 1232, because a call the graph can no longer place says so.
- A scala type alias is a type the project declares. Chasing the same rule into scala stopped on this: `NonEmptyMap` is written `type NonEmptyMap[K, +A] = NonEmptyMapImpl.Type[K, A]`, and the graph did not have it, so asking what depends on it found nothing and every rule that asks whether the project declares a name read the alias as someone else's. cats writes 106 alias names and the graph had 34; the scala node-kind table listed classes, objects, traits and enums and not `type_definition`. It has 536 more type nodes now, cats gains 32 findings as the newly-known types narrow candidates that were guessed at before, and warnings are unchanged at 329 and per project. The rule this was a prerequisite for is not written: scala's uppercase qualifiers are dominated by type parameters -- `F.map`, `G.pure` -- and telling those from a foreign object needs more than capitalisation.
